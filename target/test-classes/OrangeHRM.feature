Feature: Functional testing on Orange HRM
  ...
  Scenario Outline:  Adding a new employee - Using Scenario Outline
    ...
    Then The user wants to add login information "<username>" "<password>" "<status>"
    # "<firstName>" whatever name we provide here it needs to match with one of below columns
      # otherwise it will turn to grey
  ...
  #  Then The user wants to go to OrangeHRM Website - We cannot use steps after examples
  #  keyword because it needs to read out steps for exampelms
  #WE can leave examples empty which is going to be null by default

  #Whenever we want to automate new features we always start with typing scenarios then
  # creating steps then locating elements
